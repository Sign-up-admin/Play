...
public class ArryQueue2<E> implements Queue<E>,Iterable<E> {
//之前的ArryQueue1通过头尾指针的状态来判断环形数组的空或满
    //现在引入一个新变量来size判断元素个数
...
这会引入类型转换的不安全性，因此要小心使用。*/
    ...
    int tail=0;//尾指针
    int size=0;//元素个数，添加元素时size加1，移除元素时size-1
    @SuppressWarnings("all")//这是Java中的注解，抑制警告的产生
    public ArryQueue2(int capacity) {
